...
const cities = [
  ...
  { id: "limbe",   label: { fr: "Limbé",   en: "Limbe"   }, emoji: "🌋" },
...
export default function App() {
  ...
  const [joinForm, setJoinForm] = useState({ name:"", phone:"", skill:"plumber", zone:"", city:"yaounde", plan:"basic" });
              ...
              <div>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                  ...
                  <button className="btn"
                    onClick={async () => {
                      if (!joinForm.name.trim()||!joinForm.phone.trim()||!joinForm.zone.trim()) return;
                      await fetch(`${SUPABASE_URL}/rest/v1/workers`, {
                        method:"POST",
                        headers:{ apikey:SUPABASE_KEY, Authorization:`Bearer ${SUPABASE_KEY}`, "Content-Type":"application/json", Prefer:"return=minimal" },
                        body: JSON.stringify({ name:joinForm.name, phone:joinForm.phone, category:joinForm.skill, city:joinForm.city, zone:joinForm.zone, plan:joinForm.plan, available:true, verified:false, rating:0, review_count:0 })
                      });
                      setSubmitted(true);
                    }}
