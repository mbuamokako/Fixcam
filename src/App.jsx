              ...
              <div>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                  ...
                  <button className="btn" onClick={async () => {
                    ...
                    try {
                      const res = await fetch(`${SUPABASE_URL}/rest/v1/workers`, { method:"POST", headers:{ apikey:SUPABASE_KEY, Authorization:`Bearer ${SUPABASE_KEY}`, "Content-Type":"application/json", Prefer:"return=minimal" }, body: JSON.stringify({ name:joinForm.name.trim(), phone:joinForm.phone.trim(), category:joinForm.skill, city:joinForm.city, zone:joinForm.zone.trim(), plan:joinForm.plan, password:joinForm.password.trim(), available:true, verified:false, rating:0, review_count:0 }) });
                      if (res.ok) { setSubmitted(true); } else { const err = await res.json(); setJoinForm({...joinForm, loading:false, error: err.message||(lang==="fr"?"Erreur, réessayez.":"Error, please try again.")}); }
          ...
          <div style={{ padding:"24px 20px" }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:13 }}>
              ...
              <button className="btn" onClick={async () => {
                ...
                try {
                  const res = await fetch(`${SUPABASE_URL}/rest/v1/workers?phone=eq.${loginForm.phone.trim()}&password=eq.${loginForm.password.trim()}&select=*`, { headers:{ apikey:SUPABASE_KEY, Authorization:`Bearer ${SUPABASE_KEY}` } });
