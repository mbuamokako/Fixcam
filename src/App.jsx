...
export default function App() {
  ...
  const [requestForm, setRequestForm] = useState({ name:"", phone:"", description:"", date:"", loading:false, error:"", submitted:false });
  const [reviewForm, setReviewForm] = useState({ stars:5, comment:"", name:"", loading:false, submitted:false });
    ...
    <div className="card" onClick={() => { setSelectedArtisan(a); setPage("profile"); }}
      ...
      <div style={{ padding:"16px 16px 14px", display:"flex", gap:14, alignItems:"center" }}>
        ...
        <div style={{ position:"relative", flexShrink:0 }}>
          {a.photo_url ? (
            <img src={a.photo_url} alt={a.name} style={{ width:56, height:56, borderRadius:"50%", objectFit:"cover", boxShadow:`0 4px 14px ${a.color}55`, border:`2px solid ${a.color}33` }} />
          ) : (
    ...
    <div style={{ paddingBottom:100 }}>
      <div style={{ background:`linear-gradient(135deg,${a.color}18,${a.color}33)`, padding:"28px 20px 24px" }}>
        ...
        <div style={{ display:"flex", gap:16, alignItems:"center" }}>
          {a.photo_url ? (
            <img src={a.photo_url} alt={a.name} style={{ width:70, height:70, borderRadius:"50%", objectFit:"cover", flexShrink:0, boxShadow:`0 4px 20px ${a.color}55`, border:`3px solid ${a.color}44` }} />
          ) : (
      ...
      <div style={{ padding:"20px 20px 0" }}>
        ...
        {requestForm.submitted ? (
          <div style={{ background:"#dcfce7", borderRadius:16, padding:"20px", textAlign:"center", marginBottom:16 }}>
            <div style={{ fontSize:36, marginBottom:8 }}>✅</div>
            <div style={{ fontFamily:"'Outfit',sans-serif", fontWeight:800, fontSize:16, color:"#16a34a", marginBottom:4 }}>{lang==="fr"?"Demande envoyée!":"Request sent!"}</div>
            <div style={{ fontSize:12, color:"#16a34a" }}>{lang==="fr"?"L'artisan va vous contacter bientôt.":"The worker will contact you soon."}</div>
            <button onClick={() => setRequestForm({ name:"", phone:"", description:"", date:"", loading:false, error:"", submitted:false })}
              style={{ marginTop:12, background:"#16a34a", color:"#fff", border:"none", borderRadius:10, padding:"8px 16px", fontFamily:"'Outfit',sans-serif", fontWeight:700, fontSize:12, cursor:"pointer" }}>
              {lang==="fr"?"Envoyer une autre demande":"Send another request"}
            </button>
          </div>
        ) : (
          <div style={{ background:"#fff", borderRadius:16, padding:"18px", border:"1.5px solid #f0ece6", marginBottom:16, boxShadow:"0 4px 16px rgba(0,0,0,0.06)" }}>
            <div style={{ fontFamily:"'Outfit',sans-serif", fontWeight:700, fontSize:14, color:"#111", marginBottom:14 }}>📋 {lang==="fr"?"Envoyer une demande":"Send a Request"}</div>
            {requestForm.error && <div style={{ background:"#fee2e2", color:"#cc2200", borderRadius:10, padding:"8px 12px", fontSize:12, marginBottom:12 }}>⚠️ {requestForm.error}</div>}
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              {[
                {key:"name", label:lang==="fr"?"Votre nom":"Your name", placeholder:lang==="fr"?"Jean Dupont":"Jean Dupont", type:"text"},
                {key:"phone", label:lang==="fr"?"Votre téléphone":"Your phone", placeholder:"6XX XXX XXX", type:"tel"},
                {key:"description", label:lang==="fr"?"Décrivez votre problème":"Describe your problem", placeholder:lang==="fr"?"Ex: Fuite d'eau dans ma cuisine...":"Ex: Water leak in my kitchen...", type:"textarea"},
                {key:"date", label:lang==="fr"?"Date souhaitée (optionnel)":"Preferred date (optional)", placeholder:lang==="fr"?"Ex: Demain matin":"Ex: Tomorrow morning", type:"text"},
              ].map(f => (
                <div key={f.key}>
                  <label style={{ fontFamily:"'Outfit',sans-serif", fontWeight:600, fontSize:11, color:"#888", display:"block", marginBottom:4, textTransform:"uppercase", letterSpacing:0.5 }}>{f.label}</label>
                  {f.type==="textarea" ? (
                    <textarea value={requestForm[f.key]} onChange={e=>setRequestForm({...requestForm,[f.key]:e.target.value})}
                      placeholder={f.placeholder} rows={3}
                      style={{ width:"100%", padding:"11px 14px", borderRadius:12, border:"1.5px solid #e8e8e8", fontSize:13, resize:"none", outline:"none", background:"#fafaf8", fontFamily:"'Plus Jakarta Sans',sans-serif" }} />
                  ) : (
                    <input type={f.type} value={requestForm[f.key]} onChange={e=>setRequestForm({...requestForm,[f.key]:e.target.value})}
                      placeholder={f.placeholder}
                      style={{ width:"100%", padding:"11px 14px", borderRadius:12, border:"1.5px solid #e8e8e8", fontSize:13, outline:"none", background:"#fafaf8", fontFamily:"'Plus Jakarta Sans',sans-serif" }} />
                  )}
                </div>
              ))}
              <button className="btn" onClick={async () => {
                if (!requestForm.name.trim()||!requestForm.phone.trim()||!requestForm.description.trim()) {
                  setRequestForm({...requestForm, error: lang==="fr"?"Veuillez remplir les champs obligatoires!":"Please fill in required fields!"});
                  return;
                }
                setRequestForm({...requestForm, loading:true, error:""});
                try {
                  const res = await fetch(`${SUPABASE_URL}/rest/v1/requests`, {
                    method:"POST",
                    headers:{ apikey:SUPABASE_KEY, Authorization:`Bearer ${SUPABASE_KEY}`, "Content-Type":"application/json", Prefer:"return=minimal" },
                    body: JSON.stringify({ worker_id:a.id, client_name:requestForm.name.trim(), client_phone:requestForm.phone.trim(), description:requestForm.description.trim(), preferred_date:requestForm.date.trim(), status:"pending" })
                  });
                  if (res.ok) {
                    // Notify worker via WhatsApp
                    const msg = encodeURIComponent(`🔔 *Nouvelle demande FixCam!*

👤 Client: ${requestForm.name}
📞 Téléphone: ${requestForm.phone}
📋 Travail: ${requestForm.description}
📅 Date: ${requestForm.date||"Non précisée"}

✅ Acceptez ou refusez dans votre dashboard: fixcam.vercel.app`);
                    window.open(`https://wa.me/237${a.phone}?text=${msg}`, "_blank");
                    setRequestForm({...requestForm, loading:false, submitted:true});
                  } else {
                    setRequestForm({...requestForm, loading:false, error:lang==="fr"?"Erreur, réessayez.":"Error, please try again."});
                  }
                } catch(e) { setRequestForm({...requestForm, loading:false, error:"Connection error"}); }
              }} style={{ background:"linear-gradient(135deg,#F4A261,#ff8c42)", color:"#fff", border:"none", borderRadius:14, padding:"14px", fontFamily:"'Outfit',sans-serif", fontWeight:700, fontSize:14, boxShadow:"0 4px 16px rgba(244,162,97,0.4)" }}>
                {requestForm.loading?"...":`📤 ${lang==="fr"?"Envoyer la demande":"Send Request"}`}
              </button>
            </div>
          </div>
        )}
        ...
        {/* Review Form */}
        <div style={{ background:"#fff", borderRadius:16, padding:"18px", border:"1.5px solid #f0ece6", marginBottom:16, boxShadow:"0 4px 16px rgba(0,0,0,0.06)" }}>
          <div style={{ fontFamily:"'Outfit',sans-serif", fontWeight:700, fontSize:14, color:"#111", marginBottom:14 }}>⭐ {lang==="fr"?"Laisser un avis":"Leave a Review"}</div>
          {reviewForm.submitted ? (
            <div style={{ textAlign:"center", padding:"10px" }}>
              <div style={{ fontSize:32, marginBottom:6 }}>🎉</div>
              <div style={{ fontFamily:"'Outfit',sans-serif", fontWeight:700, fontSize:14, color:"#16a34a" }}>{lang==="fr"?"Merci pour votre avis!":"Thanks for your review!"}</div>
            </div>
          ) : (
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              <div>
                <label style={{ fontFamily:"'Outfit',sans-serif", fontWeight:600, fontSize:11, color:"#888", display:"block", marginBottom:6, textTransform:"uppercase", letterSpacing:0.5 }}>{lang==="fr"?"Note":"Rating"}</label>
                <div style={{ display:"flex", gap:8 }}>
                  {[1,2,3,4,5].map(s => (
                    <button key={s} onClick={() => setReviewForm({...reviewForm, stars:s})}
                      style={{ background:"none", border:"none", fontSize:28, cursor:"pointer", opacity:s<=reviewForm.stars?1:0.3, transition:"all 0.15s", transform:s<=reviewForm.stars?"scale(1.1)":"scale(1)" }}>⭐</button>
                  ))}
                </div>
              </div>
              <div>
                <label style={{ fontFamily:"'Outfit',sans-serif", fontWeight:600, fontSize:11, color:"#888", display:"block", marginBottom:4, textTransform:"uppercase", letterSpacing:0.5 }}>{lang==="fr"?"Votre nom":"Your name"}</label>
                <input type="text" value={reviewForm.name} onChange={e=>setReviewForm({...reviewForm,name:e.target.value})}
                  placeholder={lang==="fr"?"Jean Dupont":"Jean Dupont"}
                  style={{ width:"100%", padding:"11px 14px", borderRadius:12, border:"1.5px solid #e8e8e8", fontSize:13, outline:"none", background:"#fafaf8", fontFamily:"'Plus Jakarta Sans',sans-serif" }} />
              </div>
              <div>
                <label style={{ fontFamily:"'Outfit',sans-serif", fontWeight:600, fontSize:11, color:"#888", display:"block", marginBottom:4, textTransform:"uppercase", letterSpacing:0.5 }}>{lang==="fr"?"Commentaire":"Comment"}</label>
                <textarea value={reviewForm.comment} onChange={e=>setReviewForm({...reviewForm,comment:e.target.value})}
                  placeholder={lang==="fr"?"Décrivez votre expérience...":"Describe your experience..."} rows={3}
                  style={{ width:"100%", padding:"11px 14px", borderRadius:12, border:"1.5px solid #e8e8e8", fontSize:13, resize:"none", outline:"none", background:"#fafaf8", fontFamily:"'Plus Jakarta Sans',sans-serif" }} />
              </div>
              <button className="btn" onClick={async () => {
                if (!reviewForm.name.trim()||!reviewForm.comment.trim()) return;
                setReviewForm({...reviewForm, loading:true});
                const res = await fetch(`${SUPABASE_URL}/rest/v1/reviews`, {
                  method:"POST",
                  headers:{ apikey:SUPABASE_KEY, Authorization:`Bearer ${SUPABASE_KEY}`, "Content-Type":"application/json", Prefer:"return=minimal" },
                  body: JSON.stringify({ worker_id:a.id, reviewer_name:reviewForm.name.trim(), stars:reviewForm.stars, comment:reviewForm.comment.trim() })
                });
                if (res.ok) {
                  // Update worker rating
                  const newCount = (a.reviews||0) + 1;
                  const newRating = Math.round(((a.rating * (newCount-1)) + reviewForm.stars) / newCount * 10) / 10;
                  await fetch(`${SUPABASE_URL}/rest/v1/workers?id=eq.${a.id}`, { method:"PATCH", headers:{ apikey:SUPABASE_KEY, Authorization:`Bearer ${SUPABASE_KEY}`, "Content-Type":"application/json", Prefer:"return=minimal" }, body: JSON.stringify({ rating:newRating, review_count:newCount }) });
                  setReviewForm({...reviewForm, loading:false, submitted:true});
                }
              }} style={{ background:"#111", color:"#F4A261", border:"none", borderRadius:14, padding:"13px", fontFamily:"'Outfit',sans-serif", fontWeight:700, fontSize:13 }}>
                {reviewForm.loading?"...":`⭐ ${lang==="fr"?"Publier l'avis":"Publish Review"}`}
              </button>
            </div>
          )}
        </div>
      ...
      <div style={{ paddingBottom:85, animation:"fadeUp 0.35s ease" }}>
        ...
        {page==="home" && (
          <div>
            ...
            {/* Footer links */}
            <div style={{ padding:"20px 20px 0", display:"flex", justifyContent:"center", gap:20 }}>
              <button onClick={() => goTo("privacy")} style={{ background:"none", border:"none", color:"#aaa", fontFamily:"'Outfit',sans-serif", fontSize:11, cursor:"pointer", textDecoration:"underline" }}>
...
⏳ En attente de vérification`);
              ...
              <div>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                  ...
                  {/* Photo upload */}
                  <div>
                    <label style={{ fontFamily:"'Outfit',sans-serif", fontWeight:600, fontSize:11, color:"#888", display:"block", marginBottom:6, textTransform:"uppercase", letterSpacing:0.5 }}>📸 {lang==="fr"?"Photo de profil (optionnel)":"Profile photo (optional)"}</label>
                    <div style={{ display:"flex", gap:12, alignItems:"center" }}>
                      {joinForm.photoPreview ? (
                        <img src={joinForm.photoPreview} alt="preview" style={{ width:56, height:56, borderRadius:"50%", objectFit:"cover", border:"2px solid #F4A261" }} />
                      ) : (
                        <div style={{ width:56, height:56, borderRadius:"50%", background:"#f0ece6", display:"flex", alignItems:"center", justifyContent:"center", fontSize:22 }}>👤</div>
                      )}
                      <label style={{ background:"linear-gradient(135deg,#F4A261,#ff8c42)", color:"#fff", borderRadius:12, padding:"10px 16px", fontFamily:"'Outfit',sans-serif", fontWeight:700, fontSize:12, cursor:"pointer" }}>
                        📷 {lang==="fr"?"Choisir une photo":"Choose a photo"}
                        <input type="file" accept="image/*" style={{ display:"none" }} onChange={e => {
                          const file = e.target.files[0];
                          if (file) {
                            const reader = new FileReader();
                            reader.onload = ev => setJoinForm({...joinForm, photoFile:file, photoPreview:ev.target.result});
                  ...
                  <button className="btn" onClick={async () => {
                    ...
                    try {
                      ...
                      if (res.ok || res.status===200) {
                        ...
                        if (joinForm.photoFile) {
                          try {
                            const ext = joinForm.photoFile.name.split('.').pop();
                            const fileName = `${joinForm.phone.trim()}_${Date.now()}.${ext}`;
                            const uploadRes = await fetch(`${SUPABASE_URL}/storage/v1/object/worker-photos/${fileName}`, {
                              method:"POST",
                              headers:{ apikey:SUPABASE_KEY, Authorization:`Bearer ${SUPABASE_KEY}`, "Content-Type":joinForm.photoFile.type },
                              body: joinForm.photoFile
                            });
                            if (uploadRes.ok) {
                              const photoUrl = `${SUPABASE_URL}/storage/v1/object/public/worker-photos/${fileName}`;
                              await fetch(`${SUPABASE_URL}/rest/v1/workers?phone=eq.${joinForm.phone.trim()}`, { method:"PATCH", headers:{ apikey:SUPABASE_KEY, Authorization:`Bearer ${SUPABASE_KEY}`, "Content-Type":"application/json", Prefer:"return=minimal" }, body: JSON.stringify({ photo_url: photoUrl }) });
                            }
                          } catch(photoErr) { console.log("Photo upload failed:", photoErr); }
                        }
                        setSubmitted(true);
                      } else { const err = await res.json(); setJoinForm({...joinForm, loading:false, error: err.message||(lang==="fr"?"Erreur, réessayez.":"Error, please try again.")}); }
              ...
              <button className="btn" onClick={async () => {
                ...
                try {
                  ...
                  if (data && data.length > 0) {
                    ...
                    const reqRes = await fetch(`${SUPABASE_URL}/rest/v1/requests?worker_id=eq.${data[0].id}&select=*&order=created_at.desc`, { headers:{ apikey:SUPABASE_KEY, Authorization:`Bearer ${SUPABASE_KEY}` } });
          ...
          <div style={{ paddingBottom:20 }}>
            <div style={{ background:"linear-gradient(135deg,#1a1a1a,#2d1f0e)", padding:"28px 20px 24px" }}>
              ...
              <div style={{ display:"flex", gap:14, alignItems:"center" }}>
                <div style={{ position:"relative" }}>
                  {worker.photo_url ? (
                    <img src={worker.photo_url} alt={worker.name} style={{ width:60, height:60, borderRadius:"50%", objectFit:"cover", flexShrink:0, boxShadow:`0 4px 16px ${worker.color}55`, border:`2px solid ${worker.color}55` }} />
                  ) : (
                  ...
                  <label style={{ position:"absolute", bottom:0, right:0, width:22, height:22, borderRadius:"50%", background:"#F4A261", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", border:"2px solid #fff", fontSize:10 }}>
                    📷
                    <input type="file" accept="image/*" style={{ display:"none" }} onChange={async e => {
                      const file = e.target.files[0];
                      if (!file) return;
                      const ext = file.name.split('.').pop();
                      const fileName = `${worker.phone}_${Date.now()}.${ext}`;
                      const uploadRes = await fetch(`${SUPABASE_URL}/storage/v1/object/worker-photos/${fileName}`, {
                        method:"POST",
                      ...
                      if (uploadRes.ok) {
                        const photoUrl = `${SUPABASE_URL}/storage/v1/object/public/worker-photos/${fileName}`;
                        await fetch(`${SUPABASE_URL}/rest/v1/workers?id=eq.${worker.id}`, { method:"PATCH", headers:{ apikey:SUPABASE_KEY, Authorization:`Bearer ${SUPABASE_KEY}`, "Content-Type":"application/json", Prefer:"return=minimal" }, body: JSON.stringify({ photo_url: photoUrl }) });
                        setWorker({...worker, photo_url: photoUrl});
                      }
                    }} />
                  </label>
                </div>
            ...
            <div style={{ padding:"20px 20px 0" }}>
              ...
              {/* Requests section */}
              <div style={{ fontFamily:"'Outfit',sans-serif", fontWeight:700, fontSize:13, marginBottom:12, color:"#888", textTransform:"uppercase", letterSpacing:1, marginTop:20 }}>
                📬 {lang==="fr"?"Demandes reçues":"Received Requests"} {requests.length>0 && <span style={{ background:"#F4A261", color:"#fff", borderRadius:20, padding:"1px 8px", fontSize:10, marginLeft:6 }}>{requests.filter(r=>r.status==="pending").length}</span>}
              </div>
              {requests.length===0 ? (
                <div style={{ textAlign:"center", padding:"24px 20px", color:"#bbb", background:"#fff", borderRadius:14, border:"1.5px solid #f0ece6" }}>
                  <div style={{ fontSize:32, marginBottom:6 }}>📭</div>
                  <div style={{ fontFamily:"'Outfit',sans-serif", fontSize:12 }}>{lang==="fr"?"Aucune demande pour l'instant":"No requests yet"}</div>
                </div>
              ) : (
                <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:16 }}>
                  {requests.map(req => (
                    <div key={req.id} style={{ background:"#fff", borderRadius:14, padding:"14px 16px", boxShadow:"0 2px 8px rgba(0,0,0,0.06)", border:`1.5px solid ${req.status==="pending"?"#F4A26144":req.status==="accepted"?"#22c55e44":"#f8717144"}` }}>
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8 }}>
                        <div style={{ fontFamily:"'Outfit',sans-serif", fontWeight:700, fontSize:13, color:"#111" }}>👤 {req.client_name}</div>
                        <span style={{ background:req.status==="pending"?"#fff8f0":req.status==="accepted"?"#dcfce7":"#fee2e2", color:req.status==="pending"?"#E07B39":req.status==="accepted"?"#16a34a":"#cc2200", fontSize:9, fontWeight:700, padding:"3px 8px", borderRadius:20, fontFamily:"'Outfit',sans-serif" }}>
                          {req.status==="pending"?"⏳ En attente":req.status==="accepted"?"✅ Accepté":"❌ Refusé"}
                        </span>
                      </div>
                      <div style={{ fontSize:12, color:"#555", lineHeight:1.5, marginBottom:8 }}>{req.description}</div>
                      {req.preferred_date && <div style={{ fontSize:11, color:"#aaa", marginBottom:8 }}>📅 {req.preferred_date}</div>}
                      <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                        {req.status==="pending" && (<>
                          <button onClick={async () => {
                            await fetch(`${SUPABASE_URL}/rest/v1/requests?id=eq.${req.id}`, { method:"PATCH", headers:{ apikey:SUPABASE_KEY, Authorization:`Bearer ${SUPABASE_KEY}`, "Content-Type":"application/json", Prefer:"return=minimal" }, body: JSON.stringify({ status:"accepted" }) });
                            setRequests(requests.map(r => r.id===req.id ? {...r,status:"accepted"} : r));
                            const msg = encodeURIComponent(`✅ *Demande acceptée — FixCam*

Bonjour ${req.client_name}! Votre demande a été acceptée par ${worker.name}.

Il va vous contacter bientôt. 🔧`);
                            window.open(`https://wa.me/237${req.client_phone}?text=${msg}`, "_blank");
                          }} style={{ background:"#dcfce7", color:"#16a34a", border:"none", borderRadius:10, padding:"7px 14px", fontFamily:"'Outfit',sans-serif", fontWeight:700, fontSize:11, cursor:"pointer" }}>✅ {lang==="fr"?"Accepter":"Accept"}</button>
                          <button onClick={async () => {
                            await fetch(`${SUPABASE_URL}/rest/v1/requests?id=eq.${req.id}`, { method:"PATCH", headers:{ apikey:SUPABASE_KEY, Authorization:`Bearer ${SUPABASE_KEY}`, "Content-Type":"application/json", Prefer:"return=minimal" }, body: JSON.stringify({ status:"declined" }) });
                            setRequests(requests.map(r => r.id===req.id ? {...r,status:"declined"} : r));
                          }} style={{ background:"#fee2e2", color:"#cc2200", border:"none", borderRadius:10, padding:"7px 14px", fontFamily:"'Outfit',sans-serif", fontWeight:700, fontSize:11, cursor:"pointer" }}>❌ {lang==="fr"?"Refuser":"Decline"}</button>
                        </>)}
                        <a href={`https://wa.me/237${req.client_phone}`} target="_blank" rel="noreferrer"
                          style={{ background:"#25D36622", color:"#16a34a", borderRadius:10, padding:"7px 14px", fontFamily:"'Outfit',sans-serif", fontWeight:700, fontSize:11, textDecoration:"none" }}>
              ...
              {!worker.premium && (
                <div style={{ background:"linear-gradient(135deg,#111,#1a1a1a)", borderRadius:16, padding:"18px", marginTop:8, cursor:"pointer", border:"1px solid rgba(244,162,97,0.2)" }} onClick={() => goTo("join")}>
                  ...
                  <div style={{ fontSize:12, color:"rgba(255,255,255,0.5)" }}>{lang==="fr"?"Apparaissez en tête des recherches + badge vérifié!":"Appear at top of search + verified badge!"}</div>
                </div>
              )}
            </div>
          </div>
        ...
        {page==="privacy" && (
          <div style={{ padding:"24px 20px 40px" }}>
            <button onClick={() => goTo("home")} style={{ background:"none", border:"none", color:"#F4A261", fontFamily:"'Outfit',sans-serif", fontWeight:700, fontSize:13, cursor:"pointer", marginBottom:20, display:"flex", alignItems:"center", gap:6 }}>← {lang==="fr"?"Retour":"Back"}</button>
            <div style={{ fontFamily:"'Outfit',sans-serif", fontWeight:800, fontSize:22, color:"#111", marginBottom:4 }}>🔐 {lang==="fr"?"Politique de Confidentialité":"Privacy Policy"}</div>
            <div style={{ fontSize:11, color:"#aaa", marginBottom:24, fontFamily:"'Outfit',sans-serif" }}>FixCam · {lang==="fr"?"Dernière mise à jour: Janvier 2025":"Last updated: January 2025"}</div>

            {[
              {
                title: lang==="fr"?"1. Introduction":"1. Introduction",
                text: lang==="fr"?"FixCam est une plateforme qui connecte les artisans locaux avec des clients au Cameroun. Nous prenons la confidentialité de vos données très au sérieux. Cette politique explique comment nous collectons, utilisons et protégeons vos informations personnelles.":"FixCam is a platform connecting local workers with clients in Cameroon. We take your data privacy very seriously. This policy explains how we collect, use and protect your personal information."
              },
              {
                title: lang==="fr"?"2. Données collectées":"2. Data We Collect",
                text: lang==="fr"?"Pour les artisans: nom complet, numéro de téléphone, quartier, ville, compétences, photo de profil et mot de passe (crypté). Pour les clients: nom, numéro de téléphone et description du travail lors de l'envoi d'une demande. Nous ne collectons jamais de données bancaires ou financières directement.":"For workers: full name, phone number, neighborhood, city, skills, profile photo and password (encrypted). For clients: name, phone number and job description when sending a request. We never collect bank or financial data directly."
              },
              {
                title: lang==="fr"?"3. Utilisation des données":"3. How We Use Your Data",
                text: lang==="fr"?"Vos données sont utilisées uniquement pour: afficher votre profil aux clients, vous connecter avec des clients pertinents, améliorer nos services et vous contacter concernant votre compte. Nous ne vendons jamais vos données à des tiers.":"Your data is used only to: display your profile to clients, connect you with relevant clients, improve our services and contact you about your account. We never sell your data to third parties."
              },
              {
                title: lang==="fr"?"4. Sécurité":"4. Security",
                text: lang==="fr"?"Vos mots de passe sont cryptés avec bcrypt — même notre équipe ne peut pas les lire. Vos données sont stockées sur des serveurs sécurisés (Supabase/AWS) en Europe. Nous utilisons HTTPS pour toutes les communications.":"Your passwords are encrypted with bcrypt — even our team cannot read them. Your data is stored on secure servers (Supabase/AWS) in Europe. We use HTTPS for all communications."
              },
              {
                title: lang==="fr"?"5. Vos droits":"5. Your Rights",
                text: lang==="fr"?"Vous avez le droit de: accéder à vos données personnelles, corriger vos informations, demander la suppression de votre compte, retirer votre consentement à tout moment. Pour exercer ces droits, contactez-nous via WhatsApp au +237 695 548 173.":"You have the right to: access your personal data, correct your information, request deletion of your account, withdraw your consent at any time. To exercise these rights, contact us via WhatsApp at +237 695 548 173."
              },
              {
                title: lang==="fr"?"6. Cookies":"6. Cookies",
                text: lang==="fr"?"FixCam utilise des cookies techniques nécessaires au fonctionnement de l'application. Nous n'utilisons pas de cookies publicitaires ou de tracking.":"FixCam uses technical cookies necessary for the app to function. We do not use advertising or tracking cookies."
              },
              {
                title: lang==="fr"?"7. Contact":"7. Contact",
                text: lang==="fr"?"Pour toute question concernant cette politique de confidentialité, contactez-nous: WhatsApp: +237 695 548 173 · Email: fixcam.cm@gmail.com · Plateforme: fixcam.vercel.app":"For any questions about this privacy policy, contact us: WhatsApp: +237 695 548 173 · Email: fixcam.cm@gmail.com · Platform: fixcam.vercel.app"
              },
            ].map((section, i) => (
              <div key={i} style={{ background:"#fff", borderRadius:16, padding:"18px", marginBottom:12, border:"1.5px solid #f0ece6", boxShadow:"0 2px 8px rgba(0,0,0,0.04)" }}>
                <div style={{ fontFamily:"'Outfit',sans-serif", fontWeight:700, fontSize:14, color:"#111", marginBottom:8 }}>{section.title}</div>
                <div style={{ fontSize:13, color:"#555", lineHeight:1.7 }}>{section.text}</div>
              </div>
            ))}

            <div style={{ background:"linear-gradient(135deg,#111,#1a1a1a)", borderRadius:16, padding:"18px", marginTop:8, textAlign:"center" }}>
              <div style={{ fontFamily:"'Outfit',sans-serif", fontWeight:800, fontSize:15, color:"#fff", marginBottom:6 }}>⚡ FixCam</div>
              <div style={{ fontSize:11, color:"rgba(255,255,255,0.4)", marginBottom:12 }}>{lang==="fr"?"Connecter les artisans du Cameroun":"Connecting Cameroon's workers"}</div>
              <a href="https://wa.me/237695548173" target="_blank" rel="noreferrer"
                style={{ background:"#25D366", color:"#fff", borderRadius:10, padding:"8px 16px", fontFamily:"'Outfit',sans-serif", fontWeight:700, fontSize:12, textDecoration:"none", display:"inline-block" }}>
